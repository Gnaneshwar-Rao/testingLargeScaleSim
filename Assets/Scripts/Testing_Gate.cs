...
public class Testing_Gate : MonoBehaviour
{
    ...
    public GameObject targetGantryEntry;
    public GameObject targetGantryEnd;
    public GameObject despawnLoc;
    ...
    private void Awake()
    {
        ...
        targets.Enqueue(targetGantryEntry.transform.position);
        targets.Enqueue(targetGantryEnd.transform.position);
        targets.Enqueue(despawnLoc.transform.position);
    ...
    public IEnumerator Delay()
    {
        ...
        yield return new WaitUntil(() => agent.velocity != Vector3.zero); //waits until character starts to move
    ...
    private (float, Vector3, float) evaluateNewVel()
    {
        ...
        if (neighbourExist && tti_min < 3f)
        {
            ...
            angularVelocity = -1f * (float)Math.PI / 4;
        } else
        {
